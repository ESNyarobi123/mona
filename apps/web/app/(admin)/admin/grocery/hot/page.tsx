...
export default function GroceryHotPage() {
  ...
  function load() {
    setLoading(true);
    apiGet<HotView>("/api/admin/hot-products?module=GROCERY")
      .then((data) => {
        setView(data);
  ...
  return (
    <div className="admin-dash admin-dash--grocery">
      ...
      <p className="admin-page-lead">{t("hotGroceryDesc")}</p>
      ...
      <AdminPanel title={t("hotSettings")}>
        <div className="admin-panel__body-pad">
          ...
          <p className="admin-crud-form__hint">{t("hotSetOnProductsPage")}</p>
        </div>
      </AdminPanel>
